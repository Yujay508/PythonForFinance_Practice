...
import pandas as pd
import sys
sys.path.append('/mnt/d/7.finance_python/a.TRADING')
import smtplib
...
# 是否開盤用函數，返回字串 Y、N，Y代表有開盤，N反之
'''
target_date = 傳入datatime格式日期，為需要判定是否開盤的日期
'''
def is_open(target_date:datetime.date):
    #讀取剛剛的休市日期檔案
    hd = pd.read_excel(r"/mnt/d/7.finance_python/a.TRADING/holiday.xlsx", engine='openpyxl')
    #轉換為list備用
...
def twse_data(r_date:str):
    ...
    # 使用json套件將他 loads成 json格式之後處理
    data_json = json.loads(data.text)
    # 我們知道了欄位是 fields，資料是 data
